...
fun nms(
    ...
    val sortedDetections =
        detections.sortedWith(compareBy({ it.label }, { -it.score }))
    ...
    while (i < sortedDetections.size) {
        ...
        while (
            i < sortedDetections.size &&
            sortedDetections[i].label == currentLabel
        ) {
...
enum class CocoLabel(val id: Int) {
    ...
    companion object {
        ...
        private val idToLabel =
            values().associateBy { it.id }

        fun fromId(id: Int): CocoLabel {
            return idToLabel[id] ?: PERSON
        }
    }
}